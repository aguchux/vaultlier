...
3. Never store vault values inside `vaultlier.json` or `vaultlier.config.json`.
4. Treat `vaultlier.json` / `vaultlier.config.json` as metadata only: project ID, version, environments, key schema, scopes, defaults, and types.
...
│   └── schema/                 # shared types, Vaultlier config validation, constants
...
```txt
vaultlier.json or vaultlier.config.json
...
- Vaultlier config schema validation (`vaultlier.json` / `vaultlier.config.json`).
- Type generation.
...
- Always explain that `vaultlier.json` / `vaultlier.config.json` contains metadata only.
...
- `vaultlier.json` / `vaultlier.config.json` and `lib/vaultlier.ts` for generated artifacts.
...
- `vaultlier.json` or `vaultlier.config.json` is committed to the repository.
...
## Vaultlier Config Schema Expectations
